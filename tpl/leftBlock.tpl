            ...
            <div class="nav">
                <{if $xoops_menu_sbadmin}>
					...
					<a class="nav-link<{if $module.highlight|default:false}> active<{/if}>" href="<{$xoops_url}>" title="<{$module.name}>">
						...
						<i class="fa <{if $module.highlight|default:false}>fa-check-circle<{else}>fa-cube<{/if}>"></i>&nbsp;</div><{$smarty.const.THEME_MENU1}></a>
					
					<div class="sb-sidenav-menu-heading"><{$smarty.const.THEME_SUBMENU}></div>
					
					<a class="nav-link collapsed<{if $module.highlight|default:false}> active<{/if}>" href="#" data-bs-toggle="collapse" data-bs-target="#collapseLayouts" aria-expanded="false" aria-controls="collapseLayouts">
						<div class="sb-nav-link-icon">
